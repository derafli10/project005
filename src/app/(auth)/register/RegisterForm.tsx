 ...
 *
 * Wires the inline Server Action (passed from the Server Component page) into
 * `useActionState`, renders inline Zod validation errors, and submits via a
 * plain `<form action>`.
 *
...
export function RegisterForm({ action, labels }: RegisterFormProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        aria-describedby={
          [hasErrors ? `${id}-error` : null, hint ? `${id}-hint` : null]
            .filter(Boolean)
            .join(" ") || undefined
        }
        required
      ...
      {hint && !hasErrors ? (
        <p id={`${id}-hint`} className="text-xs text-zinc-400 dark:text-zinc-500">
          {hint}
        </p>
      ) : null}
